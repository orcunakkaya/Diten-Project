...
import { setFirstdragAndDrop, setFirstPinMode} from '../redux/actions/ditonActions';

function TableRowOne() {
    let dispatch = useDispatch();
    const [table] = useState(useSelector((state) => state.firstTable))
    const [startDrag, setStartDarg] = useState({rowBlokIndex: null, columnBlokIndex: null, itemIndex: null, color:""});
    const head = ["JAN", "FEB", "MAR"];
...
const handleDrop = (e, rowBlokIndex, columnBlokIndex, itemIndex, color) => {
    if((startDrag.rowBlokIndex === rowBlokIndex) && (startDrag.columnBlokIndex === columnBlokIndex)){
        ...
        dispatch(setFirstdragAndDrop(swap));
...
const handlePin = (row, pin, pinIndex) => {
    ...
    dispatch(setFirstPinMode(index))
    setOnPin([...pins])
}
  return (
      <div className='table'>
            <div className='table__header'>
                {
                    head.map((item, index) => (
                        <div className='table__header-item' key={index}>
                            <div className='row-count'>Q1</div>
